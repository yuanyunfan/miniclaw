...
输出 JSON 数组，每个元素 {"type", "name", "content", "confidence"}。name 不超过 30 字符，confidence 为 0 到 1。
如果没有值得记住的信息，输出空数组 []。
...
- 不要重复已有的记忆
- 简单问候、闲聊不需要提取
- 不要把 JSON 数组、JSON 对象、工具输出、日志片段或空数组 `[]` 当作 memory content
- 只输出面向长期使用的自然语言事实；如果只是不确定的中间状态，输出 []
