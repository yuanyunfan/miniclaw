...
MiniClaw 是 docs-first 项目。`pnpm run quality:docs` 当前检查第一阶段 D1 invariants：DB schema version、部分 Smart Router ER fields，以及 `docs/features/*.md` index coverage。
...
- `src/ops/doctor*`、`scripts/doctor*`
  - 要求 `docs/zh/13-auto-doctor.zh.md` 或 Auto Doctor plan/doc
- `src/stage/**`
