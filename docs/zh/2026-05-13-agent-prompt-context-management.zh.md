...
MiniClaw 当前在 chat、task、Smart Router、cron、provider、memory 和 runtime adapter 多个模块中组装 agent prompt。当前实现能跑通，但 `docs/features/19-agent-prompt-context-audit.md` 的审计发现了几类会影响后续迭代的问题：
...
- `docs/features/19-agent-prompt-context-audit.md`：当前实现审计和真实案例证据。
...
- `docs/features/03-discord-task-output.md`：只有 task trace/audit 输出影响用户可见行为时更新。
- `docs/features/04-smart-task-router.md`：只有 router input context 改变时更新。
