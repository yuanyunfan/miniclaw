...
`src/store/db.ts`目前拥有schema创建,schema版本升级,任务助手,聊天历史助手,Smart Router助手,以及多个运行时状态表. 随着MiniClaw的积累`task_events`,事件,修复运行,市场预测,智能路由器评价领域,以及未来cron运行历史,一个单一的DB模块变得难以审查.

国家生命周期也需要明确的治理。 MiniClaw储存私人业务数据:提示、微量摘要、供应商有效载荷摘要、账户相邻数据、电子邮件数据以及事件证据。 长期使用不应永远积累敏感数据。

## 目标

- 引入以下版本的迁移模块:`src/store/migrations/`.
- 增加计划移徙审计/历史。
- 从 DB 初始化中拆分表格特定寄存器的帮助。
- 添加保留策略配置和清理命令。
- 确定出口和诊断包的编辑政策。
- 使现有的用户DB升级路径安全。
...
- 不迁移到不同的数据库引擎。
- 不执行将所有寄存器分成一个承诺。
- 不删除已有的`src/store/db.ts`在呼叫站点迁移前输出。
- 在未明确配置和干运行命令的情况下,不要默默删除数据。

## 现有建筑证据

- `src/store/db.ts`: 出口`SCHEMA_VERSION`,打开 SQLite,创建表格,应用迁移,并提供许多帮助方法.
- `src/store/task-events.ts`: 已经从 DB 中分出任务事件附加/ 列表/ 计数 。
- `src/store/incidents.ts`:已经因事件、事件和修复而分道扬镳。
- `src/store/market-forecasts.ts`: 单独的市场预测寄存器.
...
- 在成功执行之后,每次迁移记录一行。
- 不得重复已应用迁移的审计行。
- 迁移失败不应突起`PRAGMA user_version`或等效的元数据。
...
- 快速预览封顶并可能被散开。
- 原始提示被默认排除。
- 提供者有效载荷不包括在内,除非存在供应商专用许可清单。
- 电子邮件/账户/经纪人字段必须由供应商专用编辑器编辑。
- 类似托肯的字符串,饼干,授权头,和会话ID总是被编辑.
- 诊断包包括一份省略/删减的字段清单。

这项政策应当通过任务追踪出口、事故中心、供应商干燥和州清理报告加以重复使用。
...
- 在旧版本升级到当前版本时测试一个DB.
- 测试二号线
2. 摘录`SCHEMA_VERSION`和迁移跑者。
- 留着`src/store/db.ts`作为公众的表面。
...
- 如果任务存储器太大,请分开。
- 使用从`db.ts`来避免大范围呼叫场面
5. 增加保留配置。
- 如果配置方案的第一个重构符没有落地, 请在当前的配置中保守添加字段 。
- 只有在项目模式需要时,才包括env覆盖。
6. 增加清理命令。
- 新的脚本`scripts/state-cleanup.ts`.
- 软件包脚本候选人`"state:cleanup": "tsx scripts/state-cleanup.ts"`.
...
- 如果共享编辑政策,更新提供者/事件/跟踪文件。
- 运行`pnpm run quality:docs`.

## 执行笔记

记录迁移版本,寄存器分割,保存默认,执行时在此进行校验命令.

### 2026-05-12 第1节:Schema迁移跑者与审计边界

- 范围:第一个DB生命周期阶段。 提取了基础创建和当前 v1-v10 迁移`src/store/db.ts`未移动任务/聊天/ Smart 路由器仓库帮助器 。
- 移徙版本:
  - `SCHEMA_VERSION = 10` in `src/store/schema.ts`.
-v1-v9 保留现有的升级行为
- 10号加了`schema_version_history`加上一个独特的`to_version`同位素审计行索引。
- 更改的文件 :
  - `src/store/db.ts`: 仍然是公共外观和再出口`SCHEMA_VERSION`; `initDb()`现在打开 SQLite, 启用 WAL, 调用`ensureBaseSchema()`,并运行版本的迁移。
  - `src/store/schema.ts`:拥有schema版本,基础schema创建,迁移跑者,历史列表,以及仅测试的迁移应用程序帮助器.
  - `src/store/migrations/*`:每个计划版本一个模块加上共享的帮助者/类型.
...
- 后续清理:拆分`tasks`, `chat_history`,以及Smart路由器决策帮助器进入寄存器模块;保留配置/清理和诊断编辑仍然是未来的阶段。
...
- 更改的文件 :
  - `src/store/db.ts`· 仍然是公共外观和再出口的存储器; 舞台现场帮手仍然在现场.
  - `src/store/__tests__/db.test.ts`: 为任务结果链接和聊天历史顺序添加直接寄存器特性测试.
  - `docs/architecture.md`, `docs/archive/2026-05-11-continuous-improvement-report.md`: 记录了寄存器边界并更新了当前热点状态.
- 核查:
  ...
  - `pnpm run build`通过; 生成忽略`dist/`文物经核实后被移除。
  - `pnpm ralph:verify -- --task complexity-hotspot-refactor --profile standard`通过。
- 公共API变化:现有消费者可以继续导入任务、聊天、智能路由器、计划以及`getDb()`帮助者来自`src/store/db.ts`.
——后续清理:保留配置,干线清理指令,共享诊断修补政策仍然是未来阶段.

### 2026-05-12 第3段:国家保留清理边界

- 范围:第三个DB生命周期阶段。 添加可配置保留和一个干运行的第一清理命令,而不修改计划版本,也不在核查时删除任何活用户数据.
- 预留:
  - `chat_history_days = 90`
  - `task_events_days = 90`
  - `smart_router_decisions_days = 180`
  ...
  - `dry_run_default = true`
- 更改的文件 :
  - `src/store/state-cleanup.ts`:增加状态清理规划,干运行保存点模拟,交易支持执行模式,分组市场预测儿童清理,以及闭关事件父母安全检查.
  ...
  - `src/store/__tests__/state-cleanup.test.ts`:覆盖干跑回滚行为,单镜清理,市场预测儿童在父母前删除,以及封闭事件安全.
  - `docs/architecture.md`, `docs/archive/2026-05-11-continuous-improvement-report.md`: 记录了保留的默认值,清理命令,以及剩余的编辑政策漏洞.
- 核查:
...
- 公共API修改:现有DB表面消费者不修改。 新的清理帮助者居住`src/store/state-cleanup.ts`;清理命令默认为干燥运行,除非配置或CLI选择执行模式.
- 后续清理:共享诊断编辑政策仍然是未来的Slice F工作。

### 2026-05-12 切片4:共享诊断修补边界

- 范围:DB生命周期/国家治理最后阶段。 增加了可重复使用的诊断校正政策,并将其连接到任务跟踪导出加Auto Doctor事件细节渲染.
- 调整政策:
  - `src/privacy/diagnostic-redaction.ts`拥有共享文本/对象编辑功能,用于授权信头、饼干、信使、即时/实体字段、原始提供者有效载荷字段、电子邮件/电话文本和会话/账户标识符。
- 会话/账户标识符改为确定相关性的短散列,而不披露原始值。
- 任务追踪导出仍为允许列表;不允许的有效载荷键继续被计算为`redacted_payload_keys`.
- 更改的文件 :
...
- 核查:
  ...
  - `pnpm run quality:docs`通过了Schema v10。
  - `pnpm ralph:verify -- --task complexity-hotspot-refactor --profile standard`通过 :`pnpm run typecheck`, `pnpm run lint`, `pnpm run quality:docs`.
- 公共 API 更改: 现有任务跟踪函数仍然导出; 输出的跟踪模型现在包含编辑的会话标识符而不是原始会话ID.
- 后续清理:DB迁移/国家生命周期计划已经完成。 未来供应商的干燥或诊断捆绑工作应再利用`src/privacy/diagnostic-redaction.ts`而不是引入提供商-本地通用秘诀。
