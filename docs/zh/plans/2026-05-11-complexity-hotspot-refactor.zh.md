...
这项计划是一个协调计划。 不在一个会话中执行下面的每一个重构 。 用它来选择一个窄片,添加或保存测试,然后更新执行笔记.
...
- 在提取过程中保持公共行为不变。
- 围绕提取出来的纯逻辑增加重点测试。
- 减少未来的爆炸半径,用于提供者、路线、修理、任务、DB和配置变化。
- 避免粗略的格式化。
...
- `src/ops/doctor-repair.ts`:政策,即时建设,工作树,特工执行,核查,允许路径检查,承诺/推.
- `src/store/db.ts`: 计划创建、迁移、任务/聊天/路由/事件/活动的储存库。
- `src/config.ts`:YAML/env 加载,验证,路径解析,运行时配置,E2E守护,许多特性配置.
...
- 增加路线决定测试,如果尚未涵盖的话。
- 重新运行智能路由器,确认,和E2E假运行时间测试。

## 切片B:`src/agent/task.ts`运行时边界

使用`2026-05-11-task-view-boundary.md`作为详细的实施计划。

保持此切片分离, 因为它会影响任务取消, 提供者流化, Discord 输出, 以及 DB 持久性 。

## 切片 C:`src/ops/doctor-scheduler.ts`医生调度器
...
## D节:`src/providers/market-intel/collectors/official.ts`
...
- `pnpm run build`
- 相关的E2E假/固定命令,例如:`pnpm run e2e:cron`或有重点的假运行时间测试。
...
- 风险:行为变化被隐藏在提取内部.
- 缓解:增加特征测试,并保护公共API。
- 回滚:返回切片承诺;如果测试失败,不要部分保留移动的代码。
- 风险:与其他计划发生冲突。
- 缓解:通过专门计划完成任务运行时间、DB和配置。
- 风险:进口货到货到货
- 缓解措施:在过渡期间保留档案和再出口旧名。
- 风险:大重构无法恢复。
- 缓解:每支部队一个单元。
...
### 2026-05-12 - Slice D Official Parser 提取器
...
- 后续清理:通过拆分继续D部分`official.ts`由源家进行管弦乐,然后通过他们的专用计划移动剩余的Slice E/F/G热点.

### 2026-05-12 - Sice D 官方来源-家庭乐团提取器
...
- 更改的文件 :
  ...
  - `src/providers/market-intel/collectors/news.ts`:提取的联邦储备局RSS端点管弦乐.
  - `src/providers/market-intel/collectors/events.ts`:提取SEC EDGAR+SSE/SZSE/HKEX公告端点管弦乐.
  - `src/providers/market-intel/collectors/scoring-input.ts`:提取出的证据部分组装,去dupe,收益/文件分割,衍生出的风险证据,以及空藏的收集构造.
  - `src/providers/market-intel/collectors/official-http.ts`, `src/providers/market-intel/collectors/official-shared.ts`: 提取取回的 HTTP 客户端, 共享源/ 结果帮助器, 失败编辑器, 以及节帮助器 。
...
- 切片名称:切片E部分,修复政策/路径/速/核查边界。
- 更改的文件 :
  - `src/ops/doctor-repair.ts`: 保留`runDoctorRepair()`, CLI 参数,结果格式化,工作树/代理/承诺/推波助澜,兼容性出口作为公共修复的表面.
  - `src/ops/doctor-repair/policy.ts`: 提取修理合格门和强制/配置处理作为无配置的纯政策模块.
  - `src/ops/doctor-repair/path-policy.ts`: 提取 git 瓷器更改文件解析 + 允许/ block glob 路径验证 。
  - `src/ops/doctor-repair/prompt.ts`: 提取的修理工用注射允许/密封的路径政策迅速渲染.
  - `src/ops/doctor-repair/verification.ts`:提取目标测试选择,标准校验命令列表,以及命令跑步循环.
  - `src/ops/__tests__/doctor-repair-boundaries.test.ts`:为提取的政策、路径、即时和核查边界增加重点特征测试。
...
- 后续清理:通过提取工作树/分支操作、Codex修复代理执行、承诺/推力助手以及报告管弦弹壳的格式,继续开展Slice E。

### 2026-05-12 - Sice E医生修复执行边界

- 切片名称:切片E完成,修复工作树/代理/报告边界。
- 更改的文件 :
  - `src/ops/doctor-repair.ts`: 保留`runDoctorRepair()`, CLI args, 兼容性导出, 事件状态过渡, 修补 运行更新, 以及高级修补管弦乐作为公共外观.
  - `src/ops/doctor-repair/worktree.ts`: 提取默认命令运行器, 修复 id sanitization, 工作树路径/分支生成, 工作树准备, 依赖安装守护, 当前 SHA 搜索, 修复 传输消息, 校验承诺, 以及孤立的分支驱动帮助器 。
  - `src/ops/doctor-repair/agent.ts`:提取Codex修复代理流,超时处理,代理响应捕获,工具日志捕获,代理故障映射.
  - `src/ops/doctor-repair/report.ts`: 提取 CLI/ 报告格式用于干运行, 政策, 更改文件, 校验, 承诺, 并按下状态输出 。
  - `src/ops/__tests__/doctor-repair-boundaries.test.ts`:增加了对已消毒的工作树目标的重点测试,工作树创建/再使用指令路由,承诺/推指令路由,以及报告格式化.
...
- 后续清理:Slice E现在已足够完整,新的修复政策、Git/worktree行为、代理流行为、核查规则和报告文本应在有重点测试的提取模块中着陆。 剩余的复杂热点工作应转移到Slice F/G或下一个明确选定的热点,而不是扩大`src/ops/doctor-repair.ts`.

### 2026-05-12 - 切片 F DB 迁移边界

- 切片名称:切片F部分、图/移徙/审计边界。
- 更改的文件 :
  - `src/store/db.ts`: 保持公共 DB 的外观,连接在其中,任务/聊天/Smart路由器帮助器,`SCHEMA_VERSION`再出口和兼容性`__testables`; 授权基准计划创建和迁移执行`src/store/schema.ts`.
  - `src/store/schema.ts`: 添加`SCHEMA_VERSION = 10`, `ensureBaseSchema()`, `runMigrations()`,计划版本检查,以及`listSchemaVersionHistory()`.
  - `src/store/migrations/*`: 提取当前 v1-v10 的 schema 迁移到版本的迁移模块外加共享列/历史帮助器.
...
- 后续清理:通过拆分继续F节`tasks`, `chat_history`,以及`smart_router_decisions`仓库帮助退出`src/store/db.ts`;保留配置、清理命令和共享诊断编辑政策仍然是今后的阶段。

### 2026-05-12 - 切片 F DB 仓库边界

- 切片名称:切片F部分,连接和任务/聊天/智能路由器寄存器边界.
- 更改的文件 :
  - `src/store/db.ts`: 减为 DB init/schema 外观, 兼容性再导出, 以及 Stage 场景帮助器; 现有导入自`src/store/db.ts`继续有效。
  - `src/store/connection.ts`:添加存储模块使用的共享直播 SQLite 手柄.
  - `src/store/repositories/tasks.ts`:提取任务行类型,任务创建,更新,查询帮助器,活动/中断/最近上市,以及智能路由器结果回写线条.
  - `src/store/repositories/chat-history.ts`: 提取聊天历史附件/列表助手.
  - `src/store/repositories/smart-router-decisions.ts`:提取智能路由器决定/审查类型和决定记录,用户选择,结果,最近,和审查帮助者.
...
- 更改的文件 :
  ...
  - `src/store/__tests__/state-cleanup.test.ts`:增加干燥回滚,单镜清理,市场预测儿童-父母前删除,封闭事件清理安全的重点测试.
  - `docs/architecture.md`, `docs/archive/2026-05-11-continuous-improvement-report.md`, `docs/plans/2026-05-11-db-migrations-state-lifecycle.md`: 记录了清理边界, 默认值, 以及剩余的编辑政策漏洞 。
- 行为平等测试:
...
- 临时DB烟雾`pnpm run state:cleanup -- --dry-run --table task_events --older-than-days 30`0名候选人通过`/private/tmp/miniclaw-state-cleanup-smoke.db`;临时DB文物经核实后被移除.
  - `pnpm ralph:verify -- --task complexity-hotspot-refactor --profile standard`通过 :`pnpm run typecheck`, `pnpm run lint`, `pnpm run quality:docs`.
- 公共API修改:现有DB表面消费者不修改。 新状态清理助手直接从`src/store/state-cleanup.ts`; CLI 默认为干线运行.
- 后续清理:共享诊断编辑政策仍然是未来的Slice F工作;更广泛的schema-first config分裂仍然属于Slice G.

### 2026-05-12 - 切片 F 共享诊断校正边界

- 切片名称:切片F完成,共享诊断编辑政策界限。
- 更改的文件 :
  - `src/privacy/diagnostic-redaction.ts`: 添加证书文本、原始即时/机构/提供者有效载荷字段、电子邮件/电话文本和散列会话/账户标识符的共享诊断编辑帮助器。
...
——后续清理:切片F现已完成. 剩余的复杂热点工作应移至Slic G 配置图谱的第一重构物或另一个明确选定的热点;未来的供应商干燥/诊断捆绑应再利用`src/privacy/diagnostic-redaction.ts`.

### 2026-05-12 - 切片 G 配置负载/Env/解析/E2E 边界采掘

- 切片名称:切片G部分,假面和纯边界提取.
- 更改的文件 :
  - `src/config.ts`: 降低到兼容性外观再导出`src/config/index.ts`.
  - `src/config/index.ts`保持运行时间配置组装,`config`, `assertE2eSafeRuntimePath()`,公共类型再导出,处理env基础URL副作用,以及当前YAML/env/默认行为.
  ...
  - `docs/architecture.md`, `docs/archive/2026-05-11-continuous-improvement-report.md`, `docs/plans/2026-05-11-config-schema-first.md`:记录了新的边界和剩余的运行时间组装工作。
- 行为平等测试:
...
- 后续清理:通过分拆继续Slice G`src/config/index.ts`进入域运行时构建器,添加更深域计划,在突变易变测试后冻结最终运行时配置对象。

### 2026-05-12 - Slice G 配置 runtime 域名构建器提取器

- 切片名称:切片 G 完成,配置运行时间/域-构建者边界.
- 更改的文件 :
  - `src/config/index.ts`:减少为公共出口和现有的代理副作用进口。
  - `src/config/runtime.ts`: 增加运行时间构成,`createRuntimeConfig()`, `config`,提供商基础 URL env 副作用保存,自动重读警告,最后的 E2E 跨战区验证,以及运行时间深冻.
  ...
  - `src/config/__tests__/config-boundaries.test.ts`:在不导入单顿配置外观的情况下,添加直接运行时间成分和深冻测试.
...
- 行为平等测试:
  ...
  - `pnpm run build`通过; 生成忽略`dist/`文物经核实后被移除。
  - `pnpm ralph:verify -- --task complexity-hotspot-refactor --profile standard`通过 :`pnpm run typecheck`, `pnpm run lint`, `pnpm run quality:docs`.
- 公共API更改:无。 现有进口`src/config.ts` / `../config.js`仍然有效;没有更改用户的YAML/env密钥形状。 运行时配置在运行时被冻结,同时保留先前的公用类型Script形状以兼容.
——后续清理:Slice G完成. 剩下的复杂工作应该转到`src/ops/doctor.ts`或单独的明确热点计划 而不是增加配置组装回`src/config/index.ts`.

### 2026-05-12 - 最终热点自动医生诊断边界

- 切片名称:最终复杂热点,只读医生诊断/证据/报告边界。 剩下的就结束了`src/ops/doctor.ts`原始背景列表中的热点; Slice B 任务运行时间完成于`docs/plans/2026-05-11-task-view-boundary.md`.
- 更改的文件 :
  ...
  - `src/ops/doctor/redaction.ts`:抽取医生-当地编辑和价值正常化帮助者。
