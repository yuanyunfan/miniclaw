...
`src/config.ts`目前处理YAML/env加载,类型强制,验证,路径解析,E2E隔离守护,代理Runtime 配置,医生/连接配置,智能路由器配置,附件/音频文本配置等等.

工程已经取决于`zod`,但主要配置路径尚未确定。 新的Runtime、Provider、传输、医生、保留和任务跟踪设置,如果每个字段都附加在一个大配置文件中,将继续提高审查成本。

## 目标

- 将配置分为负载、图案、分辨率和运行时层。
- 保护`import { config } from "../config.js"`迁移期间。
- 使新的配置字段需要计划、默认、嵌入密钥映射和测试。
- 从完全配置的导入副作用中隔离E2E护卫测试.
- 让Provider/医生/Runtime 配置可被较小的文件审查。
...
- `src/agent/runtime-config.ts`:格式Runtime 配置摘要.
...
- 浓烟:
- 装入不设配置文件的默认配置 。
- 装入`config.example.yaml`如果有helper存在或增加了helper。
...
### 2026-05-12 - 配置 load/Env/resolve/E2E 边界提取
...
- 更改的文件 :
  ...
  - `src/config/index.ts`保持Runtime 配置组装,`config`, `assertE2eSafeRuntimePath()`,公共类型 re-export,进程env 基础URL副作用,以及现有的默认/env优先行为.
  ...
  - `src/config/e2e-guard.ts`:提取纯E2E临时目录隔离检查,这样就可以在不导入Runtimesingleton 的情况下测试守卫行为.
...
- 后续清理:拆分`src/config/index.ts`由域运行时构建器,添加更深域 Zod schemas/default/env 映射测试,并在测试突变模式被移除时冻结最终Runtime 配置对象.

### 2026-05-12 - 配置运行时域建材提取器

- 范围:完成了第一道布局边界。 将最后Runtime组成拆分为`src/config/index.ts`,添加代理/Codex/Claude,路由/Smart路由器,存储/状态,任务跟踪附件,医生/连接/通知,附件/音频转录,Provider端点,E2E,和MCP,runtime-freeze 最终配置对象而不改变公共类型Script配置形状.
- 更改的文件 :
  - `src/config/index.ts`:减少为公共导出和现有的proxy side-effect import。
  - `src/config/runtime.ts`: 添加`createRuntimeConfig()`, `config`, `assertE2eSafeRuntimePath()`,深度Runtime冻结,Provider 基础 URL env 副作用保存,自动备份警告,以及最后的 E2E 跨字段验证.
  - `src/config/domains/*.ts`: 添加保留默认的域构建器, YAML 路径, env 密钥, enum/ typed 校验器, 以及每个配置域附近的路径解析度.
  - `src/config/__tests__/config-boundaries.test.ts`:在不导入singleton config外观的情况下,添加了直接运行时构成和deep-freeze覆盖.
...
- 公共API更改:无。 现有`import { config } from "../config.js"`调用点依然有效;运行时执行运行时冻结,但公用类型Script形状保持兼容,以避免将这一重构拓宽为调用点类型迁移.
