...
- 保留当前Discord行为作为默认,并保留现有的cron YAML兼容性。
- 引入具有明确能力和目标参考的IM适配器边界。
- 使用自定义的bot webhook语义法,添加飞须外出运输。
- 让克龙工作通过逻辑选择额外的IM交付`delivery_route`.
- 为当前Discord行冲过运输边界的路线回收箱。
...
- `src/runtime/im-transport.ts`定义最初的中性运输合同。
...
- `src/monitoring/recovery-outbox.ts`目前通过 Discord 冲洗待交付`Client`.

## 执行计划

1. 添加`src/im/contracts.ts`与运输身份、能力、目标、信息参考、发送/编辑/文件/线索合同有关。
...
- 缓解:遗产`channel`仍然是首要的Discord目标;`delivery_route`增加额外目标,除非呼叫者稍后明确使用仅限路由的发送.
- 风险:不成熟的入境抽象会扩大切片.
...
- 2026-05-14:执行中性合同的外出IM抽象,Discord运输适配器,Feishu自定义-bot webhook运输,逻辑`delivery_route`扇形显示 cron 消息/ 任务/ 技能结果, cron 失败额外发送, 以及 Discord 传输边界中冲刷的回收输出框 。
- 核查:`pnpm run typecheck`; `pnpm vitest run src/im/__tests__ src/cron/__tests__/runner-message.test.ts src/cron/__tests__/loader.test.ts src/monitoring/__tests__/recovery-outbox.test.ts src/config/__tests__/config-boundaries.test.ts src/__tests__/config.test.ts`(7个档案,60个测试);`pnpm run lint`; `pnpm run quality:docs`; `pnpm test`(167份文件,828项测试);`pnpm run build`.
