...
当前 repo 已经具有 TypeScript 严格模式, Vitest, 覆盖, 以及只运行的pre-commit钩`tsc --noEmit`.
...
- 添加稳定的 npm 脚本,用于commit和push 质量门。
...
- 不要叫真正的Claude/Codex, 真正的Discord, 或真正的stock provider 在commit/push时默认。
...
- 添加的package script:`typecheck`, `quality:g0`, `quality:g0:staged`, `quality:commit`, `quality:push`,并保留`e2e:discord`.
...
- 第一张G0通行证产生了预期的假阳性`.env.example`占位符和`vitest.setup.ts`测试tokenfallback; 匹配这样占位符的收紧任务`process.env`fallback不会失败树扫描 。
