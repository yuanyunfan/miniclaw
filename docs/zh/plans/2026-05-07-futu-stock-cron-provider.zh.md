...
- 配置现有的市场前和市场后股票工作,以便使用Provider。
- 保持所有账户配置`~/.miniclaw/providers/futu-stock`外边
——只保留集成内容:无交易解锁,无订单,无交易密码,无原始账户标识.
- 用注入的假客户端添加对Provider配置解析、格式化/编辑的焦点测试,以及Provider执行。

## 非目标

- 不执行交易、订单管理、战略执行或现金转移。
- 不披露原始的Futu账户行、完整的账户ID、token、cookie、电话号码或交易密码。
- 不要求 LLM 在执行时直接调用 MCP 工具 。
- 不要解决这个片段的multi-broker聚合。
- 不commit用户级别`~/.miniclaw`cron/提供 YAML 文件。
...
- Provider配置加载器来自`~/.miniclaw/providers/futu-stock/<name>.yaml`;
- Provider一级的类型;
- 以编辑的快照、报告文本、最高位置和警告等内容向JSON发出压缩;
   - `runFutuStockProvider`,可选择的依赖性注射测试。
2. registry`futu-stock` in `src/providers/index.ts`所以Cron加载器接受它。
3. 添加测试:
- 配置加载器分析默认并拒绝不安全的配置名称;
- 不以简易模式披露准确的总资产,也不以类似账户的字符串进行编辑;
- Provider可以使用假客户端运行,并返回可解析的编译JSON.
4. 添加本地用户配置 :
...
- 风险: OpenD 未运行或未来会话过期 。
- 缓解:Provider发生短暂的消毒错误,故障; cron报告`pre_provider`失败而不是伪造账户数据。
- 风险:Provider输出泄露账户识别信息或准确的资产总额。
- 缓解:再利用`redactedSnapshotJson`, `formatFutuDailyPnlReport`, `redactSensitiveText`,并添加Provider测试。
- 风险:快速超重快照P&L作为最后结算。
- 媒体:输出和提示包括P&L警告.
- 退后:删除`futu-stock`从Provider登记处删除`pre_provider`线条来自两个用户 cron YAML 文件。

## 文档同步

- 最新情况`docs/archive/features/06-futu-stock.md`与已执行的预提供器和支线连接。
- 最新情况`docs/architecture.md`Provider实例。
- 如果引入新的Provider目录,则更新README项目结构。

## 执行记录

- 已经添加了`src/providers/futu-stock`包含Provider配置加载, 每人`market_session`,紧凑JSON格式化,以及依赖注射执行测试.
- 登记`futu-stock`在提供前登记处。
- 固定共享的“未来”redactor,如此长的分数百分比不会被误认为是账户式的整数。
- 添加本地Provider配置`~/.miniclaw/providers/futu-stock/daily-stock-market.yaml`.
...
- 通过验证:
  ...
  - `pnpm test`
  - `pnpm cron:list`
- 现场Provider对当地OpenD吸烟,只打印别名/会话/位置计数。
