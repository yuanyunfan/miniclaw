...
- Do not migrate all existing `docs/archive/features/` files in the initial plan-only design change.
...
Phase 2: classify and merge `docs/archive/features/`.
...
- 2026-05-15: Completed the first Phase 3 provider merge slice for Eastmoney docs. `docs/providers/stock/eastmoney.md` is now the source-of-truth family doc for JYWG readonly and MyFavor watchlist boundaries, while `docs/archive/features/09-eastmoney-jywg-readonly-provider.md` and `docs/archive/features/17-eastmoney-myfavor-watchlist.md` are compatibility stubs for one migration cycle. `docs/README.md`, the migration map, and the Chinese Eastmoney summary were updated in the same slice; the later hardening follow-up promoted the Chinese pair to current parity.
- 2026-05-15: Completed the second Phase 3 provider merge slice for Email docs. `docs/providers/email.md` is now the source-of-truth family doc for the shared read-only Email capability, `email-query`, and `cmb-credit-card-email`; `docs/archive/features/07-email-capability.md` and `docs/archive/features/08-cmb-credit-card-email-provider.md` are compatibility stubs for one migration cycle. The provider indexes, migration map, Chinese Email summary, and website provider `source_docs` were updated in the same slice; the later hardening follow-up promoted the Chinese pair to current parity.
