...
- **Market Intel official evidence 边界**：`src/providers/market-intel/collectors/official.ts` 保留 HTTP/config/source-status orchestration；source-specific parsing、freshness、HTML/JSON/XML fixture parsing 拆到 `src/providers/market-intel/collectors/parsers/*`，用 parser fixture tests 锁定 SEC/BLS/Treasury/Fed/SSE/SZSE/HKEX/PBOC 行为
