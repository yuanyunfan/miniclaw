...
- **memories 走 markdown 不再走 SQLite 表**：`~/.miniclaw/memories/MEMORY.md` 可直接 vim 编辑、git diff、跨工具复用（hermes 同模式）。自动抽取先进入 `src/memory/curation.ts` 做候选校验、canonical key、同义去重和合并决策，再由 `src/store/memory-md.ts` 写入带 lifecycle metadata 的 Markdown；执行期 `src/memory/inject.ts` 只注入 active memory 的 `name/content`，不会把 `ttl/source/canonical_key/status` 等 metadata 传给 LLM
- **分层配置**：结构化设置优先放 `~/.miniclaw/config.yaml`；`.env` 保留 secrets、代理和临时 override；优先级是内置默认值 < YAML < env。1.0 安装模板默认使用 Codex、本机 `~` cwd、关闭 Smart Router、要求显式 @mention；新任务 runtime 首选 `runtime.default_agent`，短模型调用首选 `model.default_client`；旧 `agent.provider` / `MINICLAW_AGENT_PROVIDER` 仍作为兼容 alias 保留
- **Config runtime 边界**：`src/config.ts` 只是兼容 facade；`src/config/index.ts` 保留 public exports 和 proxy side effect；`src/config/runtime.ts` 组装并 runtime-freeze 最终 config；YAML loading 在 `src/config/load.ts`，env/type coercion reader 在 `src/config/env.ts`，Zod-backed raw object/enums 在 `src/config/schema.ts`，home/channel path resolution 在 `src/config/resolve.ts`，E2E isolation guard 在 `src/config/e2e-guard.ts`，agent/routing/storage/tasks/doctor/memory maintenance/attachments/provider/MCP runtime defaults 和 env key 映射在 `src/config/domains/*`
...
- **Task runtime 三边界**：`src/agent/runners/*-task-runner.ts` 负责 Claude / Codex / fake runtime parsing；`src/discord/task-view-reporter.ts` 负责 Discord status/progress/final output；`src/agent/task-reporter.ts` 只写 SQLite trace；`src/agent/task.ts` 负责 active task、abort、DB lifecycle 和 runtime/view/reporter wiring
- **Agent Run Manager 受控插入层**：`agent_run_manager.enabled` 默认关闭；打开后 `executeTask()` 不直接调用 selected `AgentRuntime`，而是先创建 `AgentRunManager` root run，再通过 `AgentRunScheduler` 执行 Supervisor-owned FSM plan。`agent_run_manager.auto_enabled=true` 时，`src/agent/run-manager/complexity.ts` 会用本地 deterministic complexity classifier 为 medium/high 复杂度任务自动选择 managed path；`enabled=true` 仍是显式强制开关，`enabled=false + auto_enabled=false` 仍保持 single-agent rollback path。当前默认 plan 仍是 planner -> generator -> evaluator，并支持 evaluator FAIL 后 bounded generator fix loop；但调度状态已经从 Manager 内联控制流抽出为 `agent_scheduler_state` durable snapshot，root supervisor 在等待 child event 时会进入 `waiting`，被 Agent Bus message 唤醒后恢复为 `running`。Manager 会为每个真实 Claude/Codex child run 生成 `managedContext`，把 `miniclaw-agent-bus` MCP stdio server、task/run env、guardrail env、role runtime policy、allowed tools 和 prompt usage block 注入 child runtime；Codex 通过 SDK client config override 注入 `mcp_servers.miniclaw-agent-bus`，并按 managed role policy 覆盖 task thread 的 `sandboxMode` / `approvalPolicy`：planner/evaluator 默认 `read-only`，generator 默认 `workspace-write` 且 `approvalPolicy=never`，Codex stream 中出现危险 command 或 read-only file change 时会触发 managed policy failure。Claude 通过 `mcpServers` / `allowedTools` 注入，并在 managed path 使用 role-scoped `permissionMode` / `allowedTools` / `canUseTool`：planner/evaluator 不暴露 Write/Edit/Bash/Agent，generator 才能写 workspace，且 Bash 高风险命令和 workspace 外写入会被拒绝。Claude/Codex child 仍保留 turn-end `miniclaw_agent_envelope` JSON fallback，Manager 解析后写入 `agent_messages`、`agent_artifacts`、`blackboard_facts` 并通过 scheduler wake message 恢复 root。E2E fake 模式仍走 deterministic fake planner/generator/evaluator。`src/agent/run-manager/bus.ts` 是 task-scoped typed bus，`scheduler.ts` 定义 managed scheduler plan / yield-resume runtime，`role-policy.ts` 定义 runtime-boundary role policy，`sweeper.ts` 定义 restart recovery / stale state reconciliation / TTL cleanup，`envelope.ts` 定义 fallback JSON contract，`mcp/server.ts` 暴露 `miniclaw-agent-bus` MCP tool surface，`mcp/injection.ts` 定义真实 runtime 注入 contract，`acp/*` 提供 localhost ACP-style manifest/run/message/artifact/blackboard/trace adapter。
- **Agent Run Manager DAG / synthesis / trace**：`src/agent/run-manager/scheduler.ts` 还提供 opt-in `managed-runtime-dag-v1` plan validator 和 topological execution batches，拒绝 cycle、missing dependency、unknown role、超出 `max_nodes` / `max_depth` / `max_parallel` 的 DAG；Manager 可通过 `ManagedRuntimeRunInput.schedulerPlan` 执行受控 DAG，默认 FSM path 不变。`src/agent/run-manager/final-synthesizer.ts` 生成中文 evidence-rich final result，读取 child run status、blackboard、artifact metadata、agent messages 和 verification signal，但不展开 artifact body。`src/store/agent-run-trace-export.ts` 把 scheduler、run tree、messages、artifacts、blackboard 组织成专用 managed trace section，供 `/task-log` 和 incident detail 复用。
- **Agent Run Manager guardrails**：`src/agent/run-manager/policy.ts` 定义默认限制，`agent_run_manager.{max_turns,timeout_ms,max_messages,max_artifact_bytes,max_spawn_depth,max_children_per_run,max_concurrent_runs,max_ping_pong_turns,cleanup_ttl_ms,max_fix_iterations,auto_enabled,complexity_min_score}` 可由 YAML 或 `MINICLAW_AGENT_RUN_MANAGER_*` env override。Manager 在 spawn path 强制 depth、fan-out、concurrency、total child turns 和 child timeout；Bus 在 append/write path 强制 message count、artifact byte size 和 causal ping-pong depth；`max_fix_iterations` 已从硬编码迁入 policy。`AgentRunManagerSweeper` 在 `agent_run_manager.enabled=true` 或 `agent_run_manager.auto_enabled=true` 时随 app lifecycle 启动，周期性处理 stale active runs、waiting scheduler timeout、terminal/cancelled parent child、cross-task orphan child 和 restart 后 durable wake message recovery；`cleanup_ttl_ms` 控制 terminal run-manager durable state cleanup，文件删除仅限 manager-owned `.miniclaw-task/<task_id>/artifacts/...` artifact 路径，用户 workspace file_ref 不会被删除。
- **Agent Run Manager ACP lifecycle**：`agent_run_manager.acp.enabled=false` 默认不启动 ACP。打开后，Manager 会为每个 managed task 启动 task-scoped localhost ACP server，并在 task 结束时停止；默认 host 为 `127.0.0.1`、port 为 `0`、token 为每 task ephemeral bearer token。ACP HTTP 边界执行 bearer auth、per-token/IP rate limit、payload size limit、shared diagnostic redaction error response，并可选通过 `GET /trace` 导出已脱敏 task trace Markdown。
- **Agent Run Manager store guardrails**：`src/store/agent-run-manager.ts` 维护 agent run/message/blackboard/artifact 的 typed repository，并在写入时校验 runtime/status/message kind/artifact kind/blackboard lifecycle enum、sender/receiver task 归属、artifact owner 和 source message，避免 malformed child output 直接污染共享 bus；同时提供 task message count 给 Bus 进行 durable message quota enforcement。
- **Capability contract 过渡层**：`src/runtime/agent-runtime.ts`、`model-client.ts`、`im-transport.ts` 定义 coding agent、短模型调用和 IM transport 的中性 contract；`src/agent/runtimes/registry.ts` 把 Claude/Codex task runner 包成 `AgentRuntime` shim，并用 `runtime.default_agent` 解析默认 runtime，未配置时回退 legacy `agentProvider`。当前 `/task` 已通过 registry 解析默认 `AgentRuntime` 并调用 `runtime.startTask`；E2E fake task runner 也被包装进同一个 `AgentRuntime` 边界。Smart Router LLM classifier 通过 `src/runtime/model-client-adapters.ts` 里的 `ModelClient` adapters 调用 Anthropic-compatible、OpenAI-compatible 或 read-only Codex thread；未单独配置 `routing.smart_router.llm_classifier.provider` 时回退 `model.default_client`。Auto Doctor 当前没有实际短 LLM 调用可迁移。IM 层已进入 outbound 抽象阶段：`src/im/contracts.ts` 定义 transport capabilities/target/message contract，`src/im/adapters/discord/transport.ts` 包装当前 Discord 出站能力，`src/im/adapters/feishu/transport.ts` 通过飞书 custom bot webhook 做第二 IM 的 outbound-only 验收；Discord 仍是唯一已接入的交互式 gateway。Data providers 仍作为 cron pre-provider / provider framework 独立于 AI runtime
...
- **Task trace 观测层**：`src/agent/task-reporter.ts` 把 task lifecycle、provider/tool event、Discord delivery failure 写入 `task_events`；`src/store/task-trace-export.ts` 用 payload allowlist + `src/privacy/diagnostic-redaction.ts` 生成安全 Markdown trace 和紧凑事件摘要，供 `pnpm run task:trace`、`/task-log`、Auto Doctor `/incident view` trace summary 和 `tasks.trace_auto_attach` 使用；如果 task 存在 Agent Run Manager state，会嵌入 `Agent Run Manager` managed section。raw prompt/provider payload/cookie/token/email body 不输出，session/account 类标识只保留 redacted hash
...
- **Recovery Outbox contract**：`recovery_outbox` 把本地执行结果和 IM delivery 分开。`cron_failure_alert` 保存 outage 期间没发出去的 cron failure alert，用于恢复后按频道汇总补发；可从 `cron_runs.metadata_json.failure_run_id` 或 outbox payload 还原 retry-chain id，并在 Discord 补发 summary 上附带 `立即重新执行` 按钮。`task_result_delivery` 保存 raw task 结果分片，用于恢复后补投递。当前历史 rows 仍以 Discord channel id 为主键目标，flush 通过 Discord `IMTransport` 发送，成功后把 message id 写回 outbox，并同步写回对应 `cron_runs.alert_message_id`
- **Memory maintenance contract**：`src/memory/maintenance.ts` 只维护 `~/.miniclaw/memories/MEMORY.md`，不调用 LLM、不依赖 Discord cron channel。它可删除 dirty memory、合并重复项、归档 stale memory，并为 legacy memory 补 metadata；`src/memory/maintenance-scheduler.ts` 按 `memory_maintenance.*` 后台定期执行，E2E mode 默认关闭
...
flowchart LR
    ...
    subgraph Tick["定时触发 (每分钟检查)"]
        ...
        Retry --> Guard[timeout_ms wrapper<br/>每 attempt 写 cron_runs]
        Guard --> Run[run by job.type]
    ...
    RT -->|"可选 pre_provider_preflight"| PF[runProviderHealthCheck / runProviderDryRun<br/>opt-in gate；redacted preview 不进 prompt]
...
Incident repository 查询边界在 `src/store/incidents.ts`：默认 list 仍只返回 open status set，`/incidents` 可额外按 exact status、type、severity、`diagnosis_json.category`、source provider、source route 和最新 `repair_runs.status` 过滤；查询结果按 severity 优先、再按更新时间排序，不新增 schema 字段。`/incident view` 仍从 incident 记录、recent `incident_events`、latest `repair_runs` 和 linked `cron_runs.incident_id` 拼接 operator detail；当 incident subject、linked cron run 或 source metadata 指向 task id 时，它只通过 `task-trace-export.ts` 读取安全 summary / compact event lines，并继续避免 raw evidence bundle 或私有 runtime payload。
...
`ops/doctor-repair.ts` 的执行边界是隔离 worktree + `doctor-repair/<incident-id>` 分支。它保留 CLI/scheduler facade 和 incident/repair_run 状态编排；repair eligibility 在 `ops/doctor-repair/policy.ts`，allowed/blocked path 检查在 `ops/doctor-repair/path-policy.ts`，worker prompt 在 `ops/doctor-repair/prompt.ts`，verification gate 选择与执行在 `ops/doctor-repair/verification.ts`，Codex repair agent streaming 在 `ops/doctor-repair/agent.ts`，worktree/branch/dependency/commit/push Git 操作在 `ops/doctor-repair/worktree.ts`，CLI/report 文案在 `ops/doctor-repair/report.ts`。该路径拒绝 provider auth、provider data、network、Discord、third-party 类问题，验证通过后可按配置 commit/push repair branch；它不会直接改 live main worktree。
...
`~/.miniclaw/data.db`（SQLite WAL 模式）。schema 版本使用 SQLite `PRAGMA user_version` 管理，当前版本由 `src/store/schema.ts` 的 `SCHEMA_VERSION = 14` 定义。`src/store/db.ts` 仍是 public facade，初始化时调用 `ensureBaseSchema()` 和 `runMigrations()`；live connection 由 `src/store/connection.ts` 持有，`tasks`、`chat_history`、`smart_router_decisions` 的 table-specific helper 位于 `src/store/repositories/*`，`cron_runs` 的 helper 位于 `src/store/cron-runs.ts`，Agent Run Manager 的 `agent_runs` / `agent_messages` / `blackboard_facts` / `agent_artifacts` / `agent_scheduler_state` typed helper 位于 `src/store/agent-run-manager.ts`，并继续通过 facade re-export 兼容既有 imports。版本迁移函数位于 `src/store/migrations/*`，每次成功升级会写入 `schema_version_history`。
...
诊断导出统一复用 `src/privacy/diagnostic-redaction.ts`：task trace export 只投影 allowlist payload key，`/incident view` 渲染 source/evidence/event payload 前会脱敏 authorization、cookie、token、session、account、email 和 prompt/body 字段；session/account 这类仍有排障价值的 identifier 使用短 hash，而不是明文。
