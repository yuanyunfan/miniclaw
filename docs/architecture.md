...
- **Capability contract 过渡层**：`src/runtime/agent-runtime.ts`、`model-client.ts`、`im-transport.ts` 定义 coding agent、短模型调用和 IM transport 的中性 contract；`src/agent/runtimes/registry.ts` 把 Claude/Codex task runner 包成 `AgentRuntime` shim，并继续把 legacy `agentProvider` 映射为默认 runtime。当前 `/task` 已通过 registry 解析默认 `AgentRuntime` 并调用 `runtime.startTask`；E2E fake task runner 也被包装进同一个 `AgentRuntime` 边界。Smart Router LLM classifier 通过 `src/runtime/model-client-adapters.ts` 里的 `ModelClient` adapters 调用 Anthropic-compatible、OpenAI-compatible 或 read-only Codex thread；Auto Doctor 当前没有实际短 LLM 调用可迁移，IM transport 仍在后续迁移范围内
