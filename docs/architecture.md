...
- **Agent Run Manager 受控插入层**：`agent_run_manager.enabled` 默认关闭；打开后 `executeTask()` 不直接调用 selected `AgentRuntime`，而是先创建 `AgentRunManager` root run，再按 planner -> generator -> evaluator 的 managed fallback 控制流 spawn child run。Claude/Codex child 当前通过 turn-end `miniclaw_agent_envelope` JSON 返回 summary/message/artifact/blackboard/verdict，Manager 解析后写入 `agent_messages`、`agent_artifacts`、`blackboard_facts` 并在 evaluator FAIL 时 bounded 触发 generator fix loop；E2E fake 模式仍走 deterministic fake planner/generator/evaluator。`src/agent/run-manager/bus.ts` 是 task-scoped typed bus，`envelope.ts` 定义 fallback JSON contract，`mcp/server.ts` 暴露 `miniclaw-agent-bus` MCP tool surface，`acp/*` 提供 localhost ACP-style manifest/run/message/artifact/blackboard adapter；真实 child 运行中主动调用 MCP bus 仍是后续 hardening 项。
- **Agent Run Manager store guardrails**：`src/store/agent-run-manager.ts` 维护 agent run/message/blackboard/artifact 的 typed repository，并在写入时校验 runtime/status/message kind/artifact kind/blackboard lifecycle enum、sender/receiver task 归属、artifact owner 和 source message，避免 malformed child output 直接污染共享 bus。
