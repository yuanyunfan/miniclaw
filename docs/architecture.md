...
- **Agent Run Manager 受控插入层**：`agent_run_manager.enabled` 默认关闭；打开后 `executeTask()` 不直接调用 selected `AgentRuntime`，而是先创建 `AgentRunManager` root run，再按 planner -> generator -> evaluator 的 managed fallback 控制流 spawn child run，并把 `agent_run_manager` policy 传入 Manager/Bus。Manager 会为每个真实 Claude/Codex child run 生成 `managedContext`，把 `miniclaw-agent-bus` MCP stdio server、task/run env、guardrail env、allowed tools 和 prompt usage block 注入 child runtime；Codex 通过 SDK client config override 注入 `mcp_servers.miniclaw-agent-bus`，Claude 通过 `mcpServers` / `allowedTools` 注入。Claude/Codex child 仍保留 turn-end `miniclaw_agent_envelope` JSON fallback，Manager 解析后写入 `agent_messages`、`agent_artifacts`、`blackboard_facts` 并在 evaluator FAIL 时 bounded 触发 generator fix loop；E2E fake 模式仍走 deterministic fake planner/generator/evaluator。`src/agent/run-manager/bus.ts` 是 task-scoped typed bus，`envelope.ts` 定义 fallback JSON contract，`mcp/server.ts` 暴露 `miniclaw-agent-bus` MCP tool surface，`mcp/injection.ts` 定义真实 runtime 注入 contract，`acp/*` 提供 localhost ACP-style manifest/run/message/artifact/blackboard adapter。动态 DAG/FSM scheduler、`yield_until_child_event` 恢复和 sweeper/recovery 仍是后续切片。
...
`~/.miniclaw/data.db`（SQLite WAL 模式）。schema 版本使用 SQLite `PRAGMA user_version` 管理，当前版本由 `src/store/schema.ts` 的 `SCHEMA_VERSION = 13` 定义。`src/store/db.ts` 仍是 public facade，初始化时调用 `ensureBaseSchema()` 和 `runMigrations()`；live connection 由 `src/store/connection.ts` 持有，`tasks`、`chat_history`、`smart_router_decisions` 的 table-specific helper 位于 `src/store/repositories/*`，`cron_runs` 的 helper 位于 `src/store/cron-runs.ts`，Agent Run Manager 的 `agent_runs` / `agent_messages` / `blackboard_facts` / `agent_artifacts` typed helper 位于 `src/store/agent-run-manager.ts`，并继续通过 facade re-export 兼容既有 imports。版本迁移函数位于 `src/store/migrations/*`，每次成功升级会写入 `schema_version_history`。
