...
`ops/doctor-repair.ts` 的执行边界是隔离 worktree + `doctor-repair/<incident-id>` 分支。它保留 CLI/scheduler facade、worktree/agent/commit/push orchestration；repair eligibility 在 `ops/doctor-repair/policy.ts`，allowed/blocked path 检查在 `ops/doctor-repair/path-policy.ts`，worker prompt 在 `ops/doctor-repair/prompt.ts`，verification gate 选择与执行在 `ops/doctor-repair/verification.ts`。该路径拒绝 provider auth、provider data、network、Discord、third-party 类问题，验证通过后可按配置 commit/push repair branch；它不会直接改 live main worktree。
