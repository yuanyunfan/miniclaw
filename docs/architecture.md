...
- **分层配置**：结构化设置优先放 `~/.miniclaw/config.yaml`；`.env` 保留 secrets、代理和临时 override；优先级是内置默认值 < YAML < env。新任务 runtime 首选 `runtime.default_agent`，短模型调用首选 `model.default_client`；旧 `agent.provider` / `MINICLAW_AGENT_PROVIDER` 仍作为兼容 alias 保留
...
flowchart LR
    ...
    subgraph Tick["定时触发 (每分钟检查)"]
        Reg --> Disp[dispatch<br/>按 job name 执行 max_concurrency gate]
