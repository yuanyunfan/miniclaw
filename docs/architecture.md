...
- **Recovery Outbox contract**：`recovery_outbox` 把本地执行结果和 IM delivery 分开。`cron_failure_alert` 保存 outage 期间没发出去的 cron failure alert，用于恢复后按频道汇总补发；`task_result_delivery` 保存 raw task 结果分片，用于恢复后补投递。当前历史 rows 仍以 Discord channel id 为主键目标，flush 通过 Discord `IMTransport` 发送，成功后把 message id 写回 outbox，并同步写回对应 `cron_runs.alert_message_id`
